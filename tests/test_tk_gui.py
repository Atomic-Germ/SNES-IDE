# pylint: disable=protected-access,no-name-in-module,reimported,broad-except
...
def test_tk_gui_preview_and_install(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        gui, "installer", ToolInstaller(config_file, install_dir=tmp_path / "inst")
    )
    monkeypatch.setattr(
        gui.installer, "install_selected_tools", fake_install_selected_tools
    )
    ...
    selected_calc = [
        name
        for name, var in gui.tool_vars.items()
