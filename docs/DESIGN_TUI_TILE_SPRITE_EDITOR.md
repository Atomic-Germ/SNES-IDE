...
**Status:** Phase 4 Complete (Tileset Management)  

---

## Executive Summary

This document describes the SNES tile and sprite editor built directly into the SNES-IDE TUI (Textual-based terminal interface). The editor provides a terminal-native way to create, edit, and export SNES graphics assets without requiring external GUI applications. This builds on existing Qt-based functionality in the IDE while offering the benefits of terminal accessibility, SSH compatibility, and integration with the TUI's project browser and command palette.

**Implementation Status:** Phases 1-4 are complete. The editor supports:
- Loading PNG/BMP images and creating new tilesets
- Two rendering modes (edit and display) with tile highlighting
- Full tool suite (pencil, eyedropper, fill, line, rect, ellipse)
- Undo/redo, save, and tile navigation (Tab/Shift+Tab)
- New Tileset dialog with SNES-specific options (tile size, BPP)
...
Terminal "pixels" use half-block characters (▀, ▄) or full blocks (█) with 24-bit color. The implementation uses **two rendering modes**:

**Edit Mode (zoom ≥ 4):**
- Each pixel = 2 full blocks wide (spaces with background color) × 1+ rows tall
- At zoom=4: 2 chars per pixel horizontally, 1 row per pixel vertically (square)
- At zoom=8: 4 chars per pixel horizontally, 2 rows per pixel vertically
- Easy mouse targeting for precise editing

**Display Mode (zoom < 4 or toggled):**
- Half-block rendering using `▄` with fg=bottom pixel, bg=top pixel
- Compact view: 2 pixel rows per terminal row
- Read-only overview mode with eyedropper tool

This means:
- 8x8 SNES tiles in edit mode (zoom=4) → 16x8 terminal characters
- 8x8 SNES tiles in display mode → 8x4 terminal characters
- Scrolling works in both modes via keyboard, mouse wheel, and trackpad
...
### 5.5 PixelCanvas Widget (IMPLEMENTED)

The main editing widget, extending Textual's `ScrollView` for proper scrolling support.

```python
class PixelCanvas(ScrollView, can_focus=True):
    """Terminal-based pixel canvas for SNES graphics editing.
    
    Extends ScrollView for keyboard/mouse/trackpad scrolling.
    Uses virtual_size to define scrollable content area.
    """
    
    # Reactive properties that trigger updates
    zoom = reactive(4)
    edit_mode = reactive(True)
    show_grid = reactive(True)
    current_color_index = reactive(1)
    current_tool = reactive("pencil")
    
    MIN_EDIT_ZOOM = 4  # Minimum zoom for edit mode
    
    BINDINGS = [
        Binding("ctrl+z", "undo", "Undo"),
        Binding("ctrl+y", "redo", "Redo"),
        Binding("p", "set_tool('pencil')", "Pencil"),
        Binding("i", "set_tool('eyedropper')", "Eyedropper"),
        Binding("g", "toggle_grid", "Grid"),
        Binding("d", "toggle_edit_mode", "Edit/Display"),
    ...
    def _update_virtual_size(self) -> None:
        ...
        ScrollView uses virtual_size to determine the scrollable content area.
        """
        if not self._tileset:
            self.virtual_size = Size(1, 1)
            return
        
        # Calculate content dimensions based on mode
        if self.edit_mode and self.zoom >= self.MIN_EDIT_ZOOM:
            # Edit mode: larger pixels
            chars_per_pixel = self.zoom // 2
            rows_per_pixel = max(1, self.zoom // 4)
            width = pixel_width * chars_per_pixel + grid_chars
            height = pixel_height * rows_per_pixel
        else:
            # Display mode: compact half-block
            width = pixel_width + grid_chars
            height = (pixel_height + 1) // 2
    ...
    def render_line(self, y: int) -> Strip:
        ...
        1. y is the VIEWPORT line, not content line
        2. Add scroll_offset.y to get content line
        3. Build full Strip, then crop horizontally with scroll_offset.x
        """
        scroll_x, scroll_y = self.scroll_offset
        content_y = scroll_y + y
        
        # Build segments for entire row...
        segments = []
        # [pixel rendering logic based on mode]
        
        # Crop horizontally for viewport
    ...
    def _terminal_to_pixel(self, x: int, y: int) -> Tuple[int, int] | None:
        ...
        Must add scroll_offset to convert from viewport to content space.
        """
        scroll_x, scroll_y = self.scroll_offset
    ...
    def on_mouse_down(self, event: MouseDown) -> None:
        """Handle drawing start - captures mouse for drag."""
        self.capture_mouse()
        self._drawing = True
    ...
    def on_mouse_up(self, event: MouseUp) -> None:
        """Handle drawing end - releases mouse capture."""
        self.release_mouse()
        self._drawing = False
```
...
#### 6.2.1 Tool Palette (Left Sidebar) ✅ IMPLEMENTED
Located in the collapsible "✏️ Drawing" section of the sidebar when in graphics view.

**ToolSelector Widget:**
- **Pencil** (P): Single pixel drawing ✅
- **Eyedropper** (Y): Sample colors from canvas ✅
- **Fill** (B): Flood fill ⏳
- **Line** (L): Draw straight lines ⏳
- **Rectangle** (T): Draw rectangles ⏳
- **Ellipse** (E): Draw ellipses/circles ⏳

Click a tool icon to select it, or use keyboard shortcuts.

**TilesetPreview Widget:**
- Thumbnail grid showing all tiles with average color background ✅
- Click to select a tile (for future tile-by-tile editing) ✅
- Shows tile count and hex indices ✅

#### 6.2.2 Canvas Area (Center)
- Main display surface using half-block rendering ✅ IMPLEMENTED
- Grid overlay toggle for tile boundaries ✅ IMPLEMENTED
- Selection highlight with dashed border ⏳
- Cursor showing current brush/tool ⏳

#### 6.2.3 Tileset View (Left Sidebar)
- Thumbnail grid of all tiles in tileset ✅ IMPLEMENTED
- Click to select tile for editing ✅ IMPLEMENTED
- Scroll for large tilesets ✅ IMPLEMENTED
- Highlight for currently selected tile ✅ IMPLEMENTED

#### 6.2.4 Palette Bar (Bottom) ✅ IMPLEMENTED
- 16 colors (for 4bpp) displayed as clickable swatches ✅
- Left-click to select foreground color ✅
- Selected color shown with [X] brackets ✅
- Eyedropper updates palette selection ✅
- Double-click opens color editor dialog ⏳
- Palette selector dropdown (0-7) ⏳

#### 6.2.5 Status Bar ✅ IMPLEMENTED
- Current filename and dimensions ✅
- Tile count ✅
- Zoom level ✅
- Modified indicator (yellow dot) ✅

### 6.3 Keyboard Shortcuts (IMPLEMENTED)

| Key | Action | Status |
|-----|--------|--------|
| `P` | Pencil tool | ✅ Implemented |
| `I` | Color picker (eyedropper) | ✅ Implemented |
| `G` | Toggle grid | ✅ Implemented |
| `D` | Toggle edit/display mode | ✅ Implemented |
| `+` / `=` | Zoom in | ✅ Implemented |
| `-` | Zoom out | ✅ Implemented |
| `Arrow Keys` | Scroll canvas | ✅ Implemented (via ScrollView) |
| `Page Up/Down` | Scroll canvas (large) | ✅ Implemented (via ScrollView) |
| `Home/End` | Scroll to top/bottom | ✅ Implemented (via ScrollView) |
| `Ctrl+Z` | Undo | ✅ Implemented |
| `Ctrl+Y` | Redo | ✅ Implemented |
| `Ctrl+S` | Save | ✅ Implemented |
| `Escape` | Return to TUI main view | ✅ Implemented |
| `B` | Fill tool (bucket) | ✅ Implemented |
| `L` | Line tool | ✅ Implemented |
| `T` | Rectangle tool | ✅ Implemented |
| `E` | Ellipse tool | ✅ Implemented |
| `S` | Select tool | ⏳ Future |
| `1`-`8` | Select palette 1-8 | ⏳ Future |
| `Ctrl+Shift+S` | Save As | ⏳ Future |
| `Ctrl+E` | Export to SNES format | ⏳ Future |
| `Ctrl+O` | Open file | ⏳ Future |
| `Ctrl+N` | New tileset | ⏳ Future |
| `Tab` | Next tile in tileset | ⏳ Future |
| `Shift+Tab` | Previous tile in tileset | ⏳ Future |
...
3. ✅ `PixelCanvas` widget extending `ScrollView`
4. ✅ Two rendering modes: Edit (full blocks) and Display (half-blocks)
5. ✅ Load PNG/BMP and display as tiles

**Implementation Details**:
- `PixelCanvas` extends `ScrollView` for native scrolling support
- `virtual_size` property defines scrollable content area
- `render_line(y)` uses `scroll_offset` to handle viewport positioning
- `Strip.crop()` handles horizontal scrolling
- Watch methods (`watch_zoom`, `watch_edit_mode`, `watch_show_grid`) update virtual size and refresh

**Validation**: ✅
- Can display an indexed PNG with correct colors
- Zoom works (+/- keys)
- Grid overlay toggle works (G key)
- Scrolling works via keyboard (arrows, pgup/pgdn, home/end), mouse wheel, and trackpad

### Phase 2: Basic Editing ✅ COMPLETE

**Deliverables**:
1. ✅ Pencil tool with click and drag
2. ✅ Eyedropper tool (sample colors from canvas)
3. ✅ Color picker (click palette to select color)
4. ✅ Undo/redo system
5. ✅ Save modified PNG

**Implementation Details**:
- Mouse capture/release for drag operations
- `_terminal_to_pixel()` converts viewport coords to pixel coords (with scroll offset)
- Undo stack stores deep copies of all tile pixel data
- `ColorPicked` message for palette selection
- `TileEditorPanel` container with status bar showing current color

**Validation**: ✅
- Can draw pixels on canvas in edit mode
- Can undo/redo operations (Ctrl+Z/Ctrl+Y)
- Can save modified work (Ctrl+S)
- Eyedropper (I key) picks color from canvas

### Phase 3: Additional Tools ✅ COMPLETE

**Deliverables**:
1. Fill tool (flood fill) - ✅ COMPLETE (iterative algorithm with visited set)
2. Rectangle and ellipse tools (outline) - ✅ COMPLETE (midpoint algorithm)
3. Line tool - ✅ COMPLETE (Bresenham's algorithm)
4. Selection tool (cut/copy/paste) - ⏳ Future (Phase 7 polish)
5. Flip and rotate operations - ⏳ Future (Phase 7 polish)

**UI Implementation**:
- `ToolSelector` widget in sidebar with 6 tools (functional, needs polish)
- `TilesetPreview` widget showing tile thumbnails
- `PaletteBar` widget with clickable color swatches (functional, needs polish)
- Keyboard bindings: B=fill, L=line, T=rect, E=ellipse
- Drawing section auto-expands when viewing images

**Shape Tool Preview System**:
- `_shape_start`: Starting point for line/rect/ellipse
- `_shape_preview`: Set of pixels for O(1) lookup during render
- Preview updates on mouse move, committed on mouse up
- `_get_pixel_color()` checks preview set to show preview pixels

**Validation**: ✅
- All tools work with undo/redo
- Shape preview renders correctly during drag

### Phase 4: Tileset Management ✅ COMPLETE

**Deliverables**:
1. ✅ `SNESTileset` class enhancements - Added `create_new()` class method
2. ✅ Tileset view panel with tile selection - TilesetPreview syncs with canvas
3. ✅ Navigate between tiles - Tab/Shift+Tab, TilesetPreview click scrolls canvas
4. ✅ Split image into tiles on load - Existing from Phase 1
5. ✅ New tileset dialog - `NewTilesetScreen` with SNES-appropriate options

**Implementation Details**:
- `SNESTileset.create_new(width_tiles, height_tiles, tile_width, tile_height, bpp)` creates blank tilesets with default palette
- `PixelCanvas.scroll_to_tile(tile_index)` scrolls canvas and posts `TileNavigated` message
- `current_tile_index` reactive property tracks selected tile (shown in status bar as hex)
- Grid lines adjacent to current tile highlighted in yellow
- Edit mode shows top/bottom edges with overline/underline styles
- `NewTilesetScreen` modal with:
  - Width/Height inputs (1-64 tiles)
  - Tile size select (8×8 or 16×16)
  - BPP select (2/4/8) with SNES mode hints
- Pressing 'N' opens New Tileset dialog from any view

**Validation**: ✅
- Can create new tileset with specified dimensions
- Tab navigates between tiles with visual feedback
- Clicking tile in sidebar scrolls canvas to that tile
- TilesetPreview selection syncs with canvas navigation

### Phase 5: SNES Export ⏳ PENDING
...
### Phase 6: Tilemap Support ⏳ PENDING
...
### Phase 7: Polish and Integration ⏳ PENDING
...
## Appendix A: Rendering Technical Details (IMPLEMENTED)

The SNES-IDE TUI graphics editor uses **two rendering modes**:

### A.1 Edit Mode (Full Blocks)

Used when `edit_mode=True` and `zoom >= 4`. Each pixel is rendered as multiple terminal characters for easy mouse targeting:

| Zoom | Chars/Pixel (width) | Rows/Pixel (height) | Result |
|------|---------------------|---------------------|--------|
| 4 | 2 | 1 | Square pixels, 2 chars wide |
| 8 | 4 | 2 | Larger square pixels |
| 16 | 8 | 4 | Very large pixels |

Rendering uses space characters with `bgcolor`:
```python
# For each pixel:
...
### A.2 Display Mode (Half Blocks)

Used when `edit_mode=False` or `zoom < 4`. Compact read-only view using half-block characters:

| Character | Description | Use Case |
|-----------|-------------|----------|
| `▄` (U+2584) | Lower half block | FG=bottom pixel, BG=top pixel |

Each terminal row displays 2 pixel rows:
```python
# For each horizontal pixel position:
style = Style(
    color=f"rgb({bottom_r},{bottom_g},{bottom_b})",      # FG = bottom pixel
    bgcolor=f"rgb({top_r},{top_g},{top_b})"              # BG = top pixel
)
segments.append(Segment("▄", style))
```

### A.3 Scrolling Implementation

The `PixelCanvas` extends `ScrollView` which provides native scrolling. Key implementation:

1. **Virtual Size**: Set `self.virtual_size = Size(width, height)` based on content dimensions
2. **render_line(y)**: The `y` parameter is the viewport line, add `scroll_offset.y` to get content line
3. **Horizontal Crop**: Build full row, then `strip.crop(scroll_x, scroll_x + self.size.width)`
4. **Mouse Coords**: Add `scroll_offset` to convert viewport coords to content coords

```python
def render_line(self, y: int) -> Strip:
    scroll_x, scroll_y = self.scroll_offset
    content_y = scroll_y + y
    # ... build segments for content_y ...
