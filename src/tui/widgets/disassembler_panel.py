...
class DisassemblerPanel(Container):
    ...
    def refresh_disassembly(self):
        ...
        for instr in instructions:
            ...
            # Build comprehensive comment with semantic information
            comment_parts = []
            if instr.description:
                comment_parts.append(instr.description)
            if instr.flags_affected:
                comment_parts.append(f"[{','.join(sorted(instr.flags_affected))}]")
            if instr.cycles_best != instr.cycles_worst:
                comment_parts.append(f"[{instr.cycles_best}-{instr.cycles_worst}c]")
            else:
                comment_parts.append(f"[{instr.cycles_best}c]")
            
            comment = " ".join(comment_parts) if comment_parts else instr.comment
