...
def convert() -> Literal[-1, 0]:
    ...
    try:
        ...
        if platform_manager.is_macos():
            ...
            result = subprocess_manager.run_shell_command(f'open -a "{schism}"')
        
        else:
            # Direct execution for Linux/Windows
            result = subprocess_manager.run_external_executable(schism)

        if result.failed:
            print(f"Error while executing {schism}: {result.stderr}")
            return -1
