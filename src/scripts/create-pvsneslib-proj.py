    ...
    try:
        ...
        while True:
            project_name = simpledialog.askstring(
                "Project Name",
                "Enter a name for your PVSnesLib project:\n\n"
...
class ProjectCreator:
    """Create a new PVSnesLib project with improved error handling."""
    ...
    def __init__(self) -> None:
        ...
        try:
            self.parent_dir: Path = Path(str(
                get_file_path(
                    "Select parent directory for your PVSnesLib project", 
                    [("Directories", "*")],
                    directory=True
                )
            ))
    ...
    def _get_suggested_name(self, parent_dir: Path) -> str:
        ...
        """
        base_name = "my_pvsneslib_project"
    ...
    def run(self) -> NoReturn:
        ...
        print(f"Creating PVSnesLib project '{self.project_name}'...")
        ...
        try:
            ...
            if not template_path.exists():
                raise RuntimeError(f"PVSnesLib template not found at: {template_path}")
            ...
            print(f"✓ Successfully created PVSnesLib project '{self.project_name}'")
            print(f"✓ Project location: {self.project_path}")
            print("✓ Template files copied successfully")
            print("\nYou can now open your project directory and start developing!")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create a new PVSnesLib project",
...
4. Copy the PVSnesLib template files
...
• Not start with a dot (.) on Unix systems
    ...
    parser.add_argument(
        '--version', '-v',
        action='version',
        version='PVSnesLib Project Creator v1.0'
    )
    
    args = parser.parse_args()
    
    try:
        print("=== PVSnesLib Project Creator ===")
        print("This tool will help you create a new PVSnesLib project.\n")
